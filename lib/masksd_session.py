...
def find_project_session_file(work_dir: Path) -> Optional[Path]:
    return _find_project_session_file(work_dir, ".mimo-session")
...
class MimoProjectSession:
    ...
    def mimo_session_id(self) -> str:
        sid = str(self.data.get("mimo_session_id") or "").strip()
        if sid:
            return sid
    ...
    def ensure_pane(self) -> Tuple[bool, str]:
        ...
        marker = self.pane_title_marker or "CCB-Mimo"
        resolver = getattr(backend, "find_pane_by_title_marker", None)
        if marker and callable(resolver):
            for cand in (marker, "CCB-Mimo", "CCB-MiMo", "CCB-mimo"):
                try:
        ...
        # Global discovery via mimo_comm (title contains "mimo", etc.)
        try:
            from mimo_comm import find_mimo_pane

            found_backend, found_pane = find_mimo_pane(self.data)
            if found_backend and found_pane and found_backend.is_alive(str(found_pane)):
                self.data["pane_id"] = str(found_pane)
                self.data["updated_at"] = _now_str()
                self._write_back()
                self._attach_pane_log(found_backend, str(found_pane))
                return True, str(found_pane)
        except Exception:
            pass
    ...
    def update_mimo_binding(self, *, session_id: Optional[str]) -> None:
        old_id = str(self.data.get("mimo_session_id") or "").strip()
        updated = False
        if session_id and self.data.get("mimo_session_id") != session_id:
            self.data["mimo_session_id"] = session_id
            updated = True
        if updated:
            new_id = str(session_id or "").strip()
            if old_id and old_id != new_id:
                self.data["old_mimo_session_id"] = old_id
                try:
...
def load_project_session(work_dir: Path) -> Optional[MimoProjectSession]:
    ...
    if not data:
        return None
    return MimoProjectSession(session_file=session_file, data=data)


def compute_session_key(session: MimoProjectSession) -> str:
    ...
    if not pid:
        try:
            ...
            pid = ""
    return f"mimo:{pid}" if pid else "mimo:unknown"
