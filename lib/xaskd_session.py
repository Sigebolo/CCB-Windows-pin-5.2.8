...
def find_project_session_file(work_dir: Path) -> Optional[Path]:
    return _find_project_session_file(work_dir, ".grok-session")
...
class GrokProjectSession:
    ...
    def terminal(self) -> str:
        return (self.data.get("terminal") or "wezterm").strip() or "wezterm"
    ...
    @property
    def grok_session_id(self) -> str:
        return str(self.data.get("grok_session_id") or "").strip()

    @property
    def grok_session_path(self) -> str:
        return str(self.data.get("grok_session_path") or "").strip()
    ...
    def update_grok_binding(self, *, session_id: Optional[str] = None, session_path: Optional[str] = None) -> None:
        updated = False
        if session_id and self.data.get("grok_session_id") != session_id:
            self.data["grok_session_id"] = session_id
            updated = True
        if session_path and self.data.get("grok_session_path") != session_path:
            self.data["grok_session_path"] = session_path
            updated = True
        if updated:
            self.data["updated_at"] = _now_str()
...
def load_project_session(work_dir: Path) -> Optional[GrokProjectSession]:
    ...
    if not data:
        return None
    return GrokProjectSession(session_file=session_file, data=data)


def compute_session_key(session: GrokProjectSession) -> str:
    ...
    if not pid:
        try:
            ...
            pid = ""
    return f"grok:{pid}" if pid else "grok:unknown"
