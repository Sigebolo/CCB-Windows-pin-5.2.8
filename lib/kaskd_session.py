...
def find_project_session_file(work_dir: Path) -> Optional[Path]:
    return _find_project_session_file(work_dir, ".kiro-session")
...
class KiroProjectSession:
    ...
    @property
    def kiro_session_id(self) -> str:
        return str(self.data.get("kiro_session_id") or "").strip()
...
def load_project_session(work_dir: Path) -> Optional[KiroProjectSession]:
    ...
    if not data:
        return None
    return KiroProjectSession(session_file=session_file, data=data)


def compute_session_key(session: KiroProjectSession) -> str:
    ...
    if not pid:
        try:
            ...
            pid = ""
    return f"kiro:{pid}" if pid else "kiro:unknown"
