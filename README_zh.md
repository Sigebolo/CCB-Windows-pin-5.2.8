...
> **Windows 钉扎：** 本机/团队使用的是 **Windows 可用降级基线（当前 pin：5.2.8）**，不要盲目跟随 GitHub 最新版。说明见 [docs/WINDOWS.md](docs/WINDOWS.md)。
...
### 启动
```bash
ccb                    # 按 ccb.config 启动（默认：grok 队长 + claude/mimo/opencode）
ccb grok claude        # 同时启动两个
ccb grok claude mimo opencode  # 同时启动四人小队（空格分隔）
ccb grok,claude,mimo,opencode  # 同时启动四人小队（逗号分隔）
ccb -r grok claude     # 恢复 Grok + Claude 上次会话
ccb -a grok claude mimo opencode  # 自动权限模式，启动小队
ccb -a -r grok claude mimo opencode  # 自动 + 恢复

tmux 提示：CCB 的 tmux 状态栏/窗格标题主题只会在 CCB 运行期间启用。
tmux 提示：在 tmux 内可以按 `Ctrl+b` 然后按 `Space` 来切换布局；可以连续按，多次循环切换不同布局。

布局规则：当前 pane 对应 providers 列表的**第一个**（team leader）。额外 pane 按配置顺序打开 `[cmd?, providers[1:]]`；第一个额外 pane 在右上，其后先填满左列（从上到下），再填右列（从上到下）。例：4 个 pane 左2右2，5 个 pane 左2右3。
默认小队：`grok,claude,mimo,opencode`（Grok 为队长/主控）。
...
cmd pane 作为额外 pane 参与布局，不会改变当前 pane 对应的 AI（队长仍为列表第一项）。
